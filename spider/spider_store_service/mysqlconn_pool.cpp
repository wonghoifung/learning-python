...
bool mysqlconn_pool::initconn(boost::shared_ptr<mysqlpp::Connection> conn)
{
	...
	try
	{
		while (retrycnt--)
		{
			...
			const std::string& db = config::ref().mysql_db_name();
			const std::string& ip = config::ref().mysql_server();
			const std::string& user = config::ref().mysql_user();
			const std::string& pass = config::ref().mysql_pass();
			if (conn->connect(db.c_str(), ip.c_str(), user.c_str(), pass.c_str()))
			{
				std::stringstream ssql;
				ssql << "set names utf8";
...
void mysqlconn_pool::newconn_threadfunc()
{
	while (true)
	{
		...
		else
		{
			if (!initconn(conn))
			{
				logstr("initconn failed, if u dont deal with this situation, connections will be getting fewer");
			}
			else
			{
				boost::unique_lock<boost::shared_mutex> lock(connslock_);
