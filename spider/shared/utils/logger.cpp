...
static enum logger_level g_logger_level = ll_debug;

void set_logger_level(logger_level l)
{
	logstr("logger level changed to %d", l);
	g_logger_level = l;
}
...
//		{
//			n = 0;
//			n += logger_print(line + n, linesize - n, "%p - ", buf + offset);
//
...
//				for (; i<bufleft; ++i)
//				{
//					n += logger_print(line + n, linesize - n, "%02x ", inbuf[i]);
//					++j;
//					if (4 == j)
//					{
//						j = 0;
//						n += logger_print(line + n, linesize - n, " ");
//					}
//
//				}
//				for (; i < 16; ++i)
...
//			else
//			{
//				n += logger_print(line + n, linesize,
//					"%02x %02x %02x %02x  "
//					"%02x %02x %02x %02x  "
//					"%02x %02x %02x %02x  "
//					"%02x %02x %02x %02x  ",
...
//				{
//					/* printable */
//					n += logger_print(line + n, linesize - n, "%c", *(inbuf + i));
//				}
//				else
//				{
...
FILE* logger_openfile(const std::string& basename)
{
	...
	snprintf(buf, 256, "%s%d%02d%02d%02d%02d%02d.log",
		basename.c_str(), year, month, day, hour, minute, second);
...
{
	return logger_init(basename_, fname, path);
}
        
bool logger::write(logger_level level, const std::string& prefix1, const std::string& prefix2, int prefix3, const char* fmt, ...)
{
	spinlock_guard guard(lock_);

	if (level > g_logger_level) { return true; }
...
bool logger::write_hex(logger_level level, const std::string& prefix1, const std::string& prefix2, int prefix3, const char* buf, int len)
{
	spinlock_guard guard(lock_);
...
{
	return logger_openfile(basename_);
}
